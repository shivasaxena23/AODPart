...
def experiment(proc_remote, input_data_real, pred_bin, accuracies, parameters):
  ...
  for R in parameters:
    for g in range(40):
      ...
      acc_exp[0].append([100*OPT_acc, R, "OPT"])
      acc_exp[1].append([100*AODPart_acc, R, "AODPart"])
      acc_exp[2].append([100*NeverOffload_acc, R, "NeverOffload"])
      acc_exp[3].append([100*AlwaysOffload_acc, R, "AlwaysOffload"])
      acc_exp[4].append([100*Port_acc, R, "Portend"])
      acc_exp[5].append([100*SPINN_acc, R, "SPINN"])
  return acc_exp
...
def plot(DNN_compute_values, DNN_prediction_values, input_data_real, pred_bin, accuracies, model, mode):
  ...
  parameters = np.linspace(1,2.5,10)
  ...
  df_main1 = pd.DataFrame(compiled, columns = ['Accuracy', 'R', 'Alg'])
  ...
  df_selected = df_main1.loc[df_main1['R'] == 1.5]
  ...
  h = sns.lineplot(x="R",y="Accuracy", hue="Alg", data=df_main1,style="Alg",linewidth=1, palette=['black', 'g','r','y','magenta', 'b'],
      markers=True, dashes=d_style, markersize=8, err_style="band", err_kws={'alpha':0.06})
  h.set_xticks(parameters)
  h.set_xlabel(r'$R$')
  ...
  plt.savefig("plots/"+model+"/"+model+"_R_"+mode+".pdf", bbox_inches='tight')
